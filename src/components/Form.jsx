...
function Form(props) {
  ...
  const [image, setImage] = useState(
    "https://pbs.twimg.com/media/DX5LayzUQAEWyW2.jpg"
  );
  ...
  const handleSubmit = async (e) => {
    ...
    if (params.id) {
      ...
      await axios.put(sauceURL, { fields: newSauce }, config);
    } else {
    ...
    <form onSubmit={handleSubmit}>
      <div className="input-div">
        ...
        <input
          ...
          required
          value={peppers}
          onChange={(e) => setPeppers(e.target.value)}
        ></input>
      </div>
      ...
      <div className="hotLevels">
        ...
        <input
          ...
          max={5}
          value={hotLevels}
          onChange={(e) => setHotLevels(e.target.valueAsNumber)}
        />
      </div>

      <button value={shu} onClick={newShu} className="add-button" type="submit">
        {" "}
        Add Heat{" "}
      </button>
    </form>
